    ...
    if (isCobblestoneGenerator || isBasaltGenerator || isCustomGenerator) {
        ...
        if (generatorType === 0) {
            if (tags.length > 0) {
                ...
                if (!hasTag && enableSound && config.noPermissionSound) player.runCommand(`playsound ${config.noPermissionSound}`)
                if (!hasTag) return;
            }
        ...
        if (blocks.length > 0) {
            ...
            system.runTimeout(() => {
                ...
                if (enableParticle && particle) dimension.runCommand(`particle ${particle} ${location.x} ${location.y + 1.5} ${location.z}`);
                if (enableSound && sound) dimension.runCommand(`playsound ${sound} @a ${location.x} ${location.y} ${location.z}`);
